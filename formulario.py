...
class form:
    def __init__(self):
        ...
        Label(self.win1, text="Cuantum",fg="white", background='#0d1011').pack(side=TOP, expand=1,fill=X,ipadx=5)
        Entry(self.win1,textvariable=self.cuantum,background='#0d1011', highlightbackground = "#0d1011", highlightcolor= "#fd971f",highlightthickness=0,fg="white").pack(side=TOP, expand=1,fill=X, pady=10)
        ...
        self.win1.mainloop()
    def start(self):
        if self.count_nodes.get() > 0 and self.count_nodes.get() <= 5 and self.cuantum.get() > 0:
            self.win1.destroy()
        else:
    ...
    def get_cuantum(self):
         return self.cuantum.get()
