...
class circle_list:
    ...
    def addNode(self):
        ...
        for i in range(1,self.total_pro+1):
            ...
            last_node.next.create_space(self.window,self.count_chart,self.count_table,self.total_pro,self.count_rows_tab)        
            self.count_chart += 1
            self.count_table += 1
    def create_dashboard(self):        
        Label(self.window, width=2, textvariable=self.semaforo, background='#a6e22e' ,height=1,fg="#0d1011",font=('Arial',15)).grid(column=0,row=0,padx=5,pady=5)
